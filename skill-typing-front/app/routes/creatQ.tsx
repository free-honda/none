...
const app = () => {
  ...
  const handleCreate = async () => {
    ...
    try {
      const response = await fetch("バック側のエンドポイント記載", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="container">
      <p className="mt-20 text-center text-gray-700">ジャンル</p>
      <input
        type="text"
        value={category}
      ...
      <div className="absolute right-1/3 my-2 flex">
        <button
          ...
          className="boder relative inline-block rounded border-gray-400 bg-red-400 px-1 py-1 font-semibold text-gray-800 transition-all hover:bg-red-200 active:bottom-[-1px]"
        >
          作成
        </button>
      </div>
